# MA35D05K GPIO Power Control

## Overview

NuMaker-IoT-MA35D05KI1 serves as a BMC (Dual Cortex-A35, 256MB DDR), controlling the Host power via GPIO.
...
|-------------|-----------|----------|-----|-----------|
| `POWER_OUT` | Output | Emulates ATX power button (200ms=power on, 4s=force off) | PC2 | gpioc, line 2 |
| `RESET_OUT` | Output | Emulates reset button (pull low for 500ms) | PC3 | gpioc, line 3 |
| `PS_PWROK` | Input | Detects Host PSU Power Good | PC7 | gpioc, line 7 |

## Wiring Diagram

```
MA35D05K (BMC)                 Host Motherboard
──────────────                 ────────────────
PC2 (POWER_OUT)  ─────────── PWR_BTN# (power button header)
PC3 (RESET_OUT)  ─────────── RST_BTN# (reset button header)
PC7 (PS_PWROK)   ◄────────── PWROK (ATX PSU / VRM output)
GND ──────────────────────── GND (common ground)
```

> **Electrical Specs**: MA35D05K GPIO is 3.3V. POWER_OUT/RESET_OUT are active-low (Host side has pull-up resistors). PS_PWROK is active-high input.

## DTS Configuration

GPIO line names are defined in `ma35d05k-iot-ma35d05ki1-v1-256m.dts`:
...
```bash
# List all gpiochips (MA35D05K has one per port)
gpiodetect
...
|-----------|-----|-------------|----------------|----------------|
| Power On | Redfish `On` / D-Bus `Transition.On` | **PC2** (CH1) | Active LOW | 200ms |
| Graceful Shutdown | Redfish `GracefulShutdown` / D-Bus `Transition.Off` | **PC2** (CH1) | Active LOW | 200ms (first) |
| Force Off | Redfish `ForceOff` | **PC2** (CH1) | Active LOW | 4000ms |
| Force Restart | Redfish `ForceRestart` / D-Bus `Transition.Reboot` | **PC3** (CH2) | Active LOW | 500ms |
| Power Detection | Query Chassis PowerState | **PC7** (CH3) | Read DC level | HIGH=On, LOW=Off |

## Notes

- All three signals (PC2, PC3, PC7) are on the same GPIO port (Port C) — convenient for hardware routing
