...
# --- Configuration Area ---
BMC_IP="numaker-iot-ma35d05ki1.local"  # mDNS hostname (avahi)
USER="root"
PASS="0penBmc"
REPORT_FILE="$(dirname "$0")/numaker-iot-ma35d05ki1.md"
# -------------------------
...
echo -e "${BLUE}====================================================${NC}"
echo -e "     MA35D05K OpenBMC Functions Integration Test      "
echo -e "${BLUE}====================================================${NC}"
...
# 5. Toggle Identify LED (PB8) to Blink (LocationIndicatorActive = true)
echo -e "\n${YELLOW}>>> Action: Enabling Identify LED (PB8 Blink) <<<${NC}"
...
# 7. Turn off Identify LED (PB8)
echo -e "\n${YELLOW}>>> Action: Disabling Identify LED (PB8 Off) <<<${NC}"
...
echo -e "${RED}│     Wiring: PC7 → VDD 3.3V or motherboard PWR_LED+        │${NC}"
echo -e "${RED}└─────────────────────────────────────────────────────────────┘${NC}"
read -p "  Press Enter to confirm PC7=HIGH is ready (or PWR_LED+ connected)..."
report "- **Power On**: Operator prompted to connect PC7 to HIGH"
...
echo -e "${RED}│     PC7 should return to LOW (0V) via pull-down resistor   │${NC}"
echo -e "${RED}└─────────────────────────────────────────────────────────────┘${NC}"
read -p "  Press Enter to confirm ready to set PC7=LOW after ForceOff..."
report "- **Force Off**: Operator prompted to disconnect PC7 to LOW"
...
echo -e "${RED}│     PC7 (PS_PWROK) should remain HIGH (Host stays on)      │${NC}"
echo -e "${RED}│     If PC7=LOW, connect HIGH first before continuing       │${NC}"
echo -e "${RED}└─────────────────────────────────────────────────────────────┘${NC}"
read -p "  Press Enter to confirm PC7=HIGH (Host is powered on)..."
report "- **Force Restart**: Confirmed PC7=HIGH"
