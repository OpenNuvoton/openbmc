...
USER="root"
PASS="0penBmc"
REPORT_FILE="$(dirname "$0")/numaker-iot-nuc980g2.md"
# -------------------------
...
echo -e "${RED}┌─────────────────────────────────────────────────────────────┐${NC}"
echo -e "${RED}│  ⚡ POWER ON: After sending command, connect PF9 (PS_PWROK) │${NC}"
echo -e "${RED}│     to 3.3V HIGH (simulate Host PSU Power Good)            │${NC}"
echo -e "${RED}│     Wiring: PF9 → VDD 3.3V or motherboard PWR_LED+       │${NC}"
echo -e "${RED}└─────────────────────────────────────────────────────────────┘${NC}"
read -p "  Press Enter to confirm PF9=HIGH is ready (or PWR_LED+ connected)..."
report "- **Power On**: Operator prompted to connect PF9 to HIGH"
...
echo -e "\n${BLUE}[Verify] Confirm PF9 (PS_PWROK) reads HIGH → Chassis=On${NC}"
...
echo -e "${RED}┌─────────────────────────────────────────────────────────────┐${NC}"
echo -e "${RED}│  ⚡ FORCE OFF: After sending command, disconnect PF9       │${NC}"
echo -e "${RED}│     (PS_PWROK) from 3.3V (remove PWR_LED+ wire)            │${NC}"
echo -e "${RED}│     PF9 should return to LOW (0V) via pull-down resistor   │${NC}"
echo -e "${RED}└─────────────────────────────────────────────────────────────┘${NC}"
read -p "  Press Enter to confirm ready to set PF9=LOW after ForceOff..."
report "- **Force Off**: Operator prompted to disconnect PF9 to LOW"
...
sleep 5
echo -e "${RED}  >>> Please confirm PF9 is now LOW (3.3V disconnected) <<<${NC}"
read -p "  Press Enter to confirm PF9 is LOW..."
...
echo -e "${RED}┌─────────────────────────────────────────────────────────────┐${NC}"
echo -e "${RED}│  ⚡ RESTART: RESET_OUT (PD13) will pulse LOW for 500ms     │${NC}"
echo -e "${RED}│     PF9 (PS_PWROK) should remain HIGH (Host stays on)      │${NC}"
echo -e "${RED}│     If PF9=LOW, connect HIGH first before continuing       │${NC}"
echo -e "${RED}└─────────────────────────────────────────────────────────────┘${NC}"
read -p "  Press Enter to confirm PF9=HIGH (Host is powered on)..."
report "- **Force Restart**: Confirmed PF9=HIGH"
