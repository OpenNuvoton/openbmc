...
DESCRIPTION = "Produces a nuwriter-pack.bin that can be programmed to SPI NAND via NuWriter tool"
LICENSE = "MIT"
...
do_compile() {
    ...
    bbnote "Generating NUC980 nuwriter-pack.bin..."
    python3 ${S}/nuwriter_pack.py -o ${B}/nuwriter-pack.bin \
    ...
    if [ ! -f ${B}/nuwriter-pack.bin ]; then
        bbfatal "nuwriter_pack.py failed to generate nuwriter-pack.bin"
    fi
    bbnote "NuWriter nuwriter-pack.bin generated: $(ls -lh ${B}/nuwriter-pack.bin)"
}

do_deploy() {
    install -d ${DEPLOYDIR}
    install -m 0644 ${B}/nuwriter-pack.bin ${DEPLOYDIR}/nuwriter-pack.bin
